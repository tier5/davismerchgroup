<?php
        ...
        require('martin_mailfunctions.php');
$to        = 'To Email ID';
$message   = 'Type in message here';
$subject   = 'Test ( '.$mailServerAddress.' )';
$error     = '';
...
if (isset($_POST['send']) && $_POST['send'] == 'Send')
{
    ...
    if ($error == '')
    {
        ...
        $headers = create_smtp_headers(trim($subject), "admin@davismerchgroup.com", trim($to), 'Web Admin', "", "text/html");
        $data    = $headers . "<html><body><p>" . nl2br($message) . "</p></body></html>";
        if ((send_smtp($mailServerAddress,"admin@davismerchgroup.com", trim($to), $data)) === false)
        {
            $msg_class = 'style="color:red;font-weight: bold"';
		global $last_output;
