...
//$where = ' where  prj.status =1 and '.$type.'=(select region from "employeeDB" where "employeeID"='.$_SESSION['employeeID'].') ';  //$client_sql";
...
if(isset($_GET['project'])&&$_GET['project']!="")
{
 ...
 $where.=" and main.m_pid =".$_GET['project']."";   
 //$join_type='inner join';
 $cl_flag=1;
}
...
$sql ='SELECT distinct ( prj.prj_name ),main.name, prj.pid, chain.chain,m1.firstname,reg_name.region , m1.lastname, st.sto_num, m.st_time,m.due_date 
...
left join "employeeDB" as m1 on m1."employeeID"= m.merch left join tbl_chainmanagement as st on st.chain_id = m.store_num  '.
   ' left join project_main as main on main.m_pid=prj.m_pid left join tbl_region as reg_name on reg_name.rid=m.region '.
...
<table width="100%" cellspacing="1" cellpadding="1" border="0">
  <tr><td colspan="4"><h2>Email Weekly Schedule</h2></td></tr>  
<tr><td colspan="5">&nbsp;</td></tr>
 ...
 <option value="" selected>--------Select--------</option>                   
    ...
    <input class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="button" value="Generate spreadsheet" onclick="spreadSheet();"/>
    <input class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" onclick="send_report();" type="button" value="Send Email"/>
        <input  class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" onclick="list_report();" type="button" value="Search"/>
     <input  class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" onclick="cancel_search();" type="button" value="Cancel"/> 
        ...
        <?php
			  ...
			  ?>
                                <tr>
                                   ...
                                   <td class="gridVal"><?php if($datalist[$i]['due_date']>0)echo date('l, d- M',$datalist[$i]['due_date']);?></td>
        ...
        <?php
			...
			<tr><td width="100%" class="grid001" colspan="13">' . $p->show() . '</td></tr>			
			  ...
			  {
				  echo '<tr><td colspan="7" class="gridVal">No Job found</td><tr>';
			  }
			 ?>       
                        <tr>
                          <td colspan="5">&nbsp;</td>
...
{
alert('Plese select a date range and try again..');
...
{
 var data=$('#srch_frm').serialize();
 location.href='<?php echo $mydirectory;?>/admin/email_weekly/index.php?'+data;
}

function cancel_search()
{
 location.href='<?php echo $mydirectory;?>/admin/email_weekly/index.php';
}
</script>  
