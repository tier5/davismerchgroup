...
$sql = 'SELECT  store.city, '.
	...
	't.client as t_client, '.
	't.lunch_time as lunch_time, '.
	't.other as other, '.
	't.dt_hours as dt_hours, '.
	't.pid as pid, '.
	't.m_id as m_id, '.
	'TO_CHAR(TO_TIMESTAMP(t.clockin), \'MM/DD/YYYY\') as st_date, '.
	'TO_CHAR(TO_TIMESTAMP(t.clockin), \'HH12:MI AM\') as st_time, '.
	'TO_CHAR(TO_TIMESTAMP(t.clockout), \'HH12:MI AM\') as end_time, '.
	'TO_CHAR(TO_TIMESTAMP(t.clockout), \'YYYY\') as end_time_yr, '.
	't.other as others, '.
	't.store as store, '.
	'c."ID" as c_id, '.
	'c.client as client, '.
	...
	'LEFT JOIN "clientDB" AS c ON t.cid = c."ID" '.
...
$sql.=' left join "tbl_chainmanagement" as store on store.chain_id = t.store_num '.
//" left join tbl_store as store on t.store_num like cast(store.sid as character varying)"
        ' left join tbl_chain as ch on t.store = ch.ch_id ';
 ...
 if(isset($_REQUEST['start_dt']))
{
	...
	if($start)
	{
		$search_sql .= " and t.clockin >= '".strtotime($_REQUEST['start_dt'])."'";
...
if(isset($_REQUEST['end_dt']))
{
	...
	if($end)
	{
		$search_sql .= " and t.clockin <= '".strtotime($_REQUEST['end_dt'])."'";
