...
{
    $sql = "delete from dtbl_odometer where time_id=".$_GET['delete'].";delete from timeclock_new where id=".$_GET['delete']; 
...
if(isset($_POST['action']))
{
	...
	{
            if(isset($delete_x))
               $sql .= "delete from dtbl_odometer where time_id=".$id.";delete from timeclock_new where id=".$id.";"; 
                else
		$sql .= " UPDATE timeclock_new set status = $status_val where id=$id ;";
	}       
	if(!($result=pg_query($connection,$sql))){
...
$sql="Select store.city, ".
	...
	"left join \"tbl_chainmanagement\" as store on store.chain_id = t.store_num ".
