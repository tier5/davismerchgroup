...
$sql = 'SELECT  store.sto_num as store_num_val,ch.chain,t.*,odo.*,e.firstname,e.lastname,odo.daily_total as daily_total,odo.other_exp as other_exp,e.city as city FROM dtbl_timesheet as t left join "employeeDB" as e on e."employeeID" = t.emp_id left join dtbl_odometer as odo on odo.time_id=t.time_id  ';  
...
for($i=0; $i < count($datalist); $i++)
{
   $travel=$datalist[$i]['misc_exp'];
	...
	$content .= '"'.rtrim(str_replace('"','""',$datalist[$i]['chain'].'-'.$datalist[$i]['store_num_val'])).'",'; 
