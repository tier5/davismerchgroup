...
<form action="project_submit.php" id="store_frm" method="post">
  <table align="center">
         <tr>
            ...
            <td><input id="prj_name" type="text" name="proj[prj_name]" value="<?php echo stripslashes($datalist['prj_name']); ?>" size="20">
            <input type="hidden" name="pid" value="<?php echo $datalist['pid'];?>"/>
            </td>
        </tr>  
      
      
        <tr>
            <td align="right">Date of completion:</td>
            <?php 
            $date_hrd='';
         if(isset($datalist['date_complet']))   
            $date_hrd=date('m/d/Y',$datalist['date_complet']);
            ?>
            <td><input type="text" id="date_complet" name="proj[date_complet]" value="<?php echo stripslashes($date_hrd); ?>" size="20"></td>
        </tr>  
        <tr>
            <td align="right">Description:</td>
            <td><input type="text" name="proj[description]" value="<?php echo stripslashes($datalist['description']); ?>" size="20"></td>
        </tr>
        
            <tr>
            <td valign="top" align="right">Sign Off Forms:</td>
            <td><input type="hidden" name="hdn_sign_id[0]" value="<?php echo $data_signoff[0]['sign_id'];?>"/>
              <?php $frm_arr=array("dmg_form","dmg_convenience_form","stater_bros_form","frito_lay_rest_form","pizza_form"); 
         $frmname_arr=array("DMG Chain Form","DMG Convenience Form","Grocery Form","Frito-Lay Form","Nestle Form"); ?>  
                <select class="sign_off_sel" id="sign_off_sel" name="sign_off[0]" >
                  <?php  foreach($frm_arr as $key=>$frm){?>
         <option <?php if($data_signoff[0]['frm_name']==$frm) echo ' selected ';
?>value="<?php echo $frm;?>"><?php echo $frmname_arr[$key];?></option>
<?php }?>
                </select>&nbsp;<img width="20px" height="20px" src="<?php echo $mydirectory;?>/images/add.png"
                          title="Add signoff" onclick="javascript:sign_off_add();"/>
           ...
           <td align="right">client:</td>
			...
			for ($i = 0; $i < count($client); $i++) {
    			echo '<option value="'.$client[$i]['ID'].'" ';
    				if (isset($datalist['client']) && $datalist['client'] == $client[$i]['ID'])
...
&nbsp;<img width="20px" height="20px" src="<?php echo $mydirectory;?>/images/delete.png"
