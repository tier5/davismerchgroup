...
if(isset($_GET['del'])&&$_GET['pid']>0)
{
    $sql="select * from projects where m_pid = ".$_GET['pid'];
      //  echo $sql;
    ...
    {
  echo '<h3 style="color:red;" align="center">Some jobs are existing for this project,delete them and try again...</h3>';      
    }
    else{
        pg_free_result($result);
        
        
	$sql="Delete from prj_signoff_clients where pid =".$_GET['pid'].";Delete from project_main where m_pid = ".$_GET['pid'];
      //  echo $sql;
	if(!($result=pg_query($connection,$sql)))
	{
		print("Failed delete_quote: " . pg_last_error($connection));
...
if(isset($_GET['ch_name'])&& $_GET['ch_name']!="")
{
 ...
 $where=" where p.pid ='".$_GET['ch_name']."'";   
}

$sql = 'SELECT DISTINCT p.*,cl.client as client_name FROM "project_main" as p 
    left join prj_signoff_clients as sign_cl on sign_cl.pid=p.m_pid '.
    ' left join "clientDB" as cl on cl."ID"=sign_cl.client   '.$where.' ORDER BY m_pid DESC';
/*if($where!="")
    $sql.= $where;*/
//echo $sql;
...
echo "<blockquote>";
echo "<center><font size=\"5\">Project Management</font><br/><br/>";
echo "</blockquote>";
echo "</font>";
?>
<table width="100%"> 
            ...
            <tr>
                <td align="center" valign="top"><font size="5"><br>
                    <table border="0" width="20%">
                        <tr>
                            <td align="center"><input type="button" onmouseover="this.style.cursor = 'pointer';" value="Add New Project" onclick="location.href='add_project.php';" /></td>
<td align="center"><input type="button" onmouseover="this.style.cursor = 'pointer';" value="Job Page" onclick="location.href='<?php echo $mydirectory;?>/admin/projects/index.php';" /></td>   
                        </tr>
                    </table>
                    <br>
                    </font>
...
<td class="grid001" width="75px" bgcolor="C0C0C0"><b>Project:</b> </td>
    <td class="grid001"><select name="ch_name" id="ch_name"> 
    <option value="">--------SELECT---------</option>                
                   <?php
			for ($i = 0; $i < count($data_proj); $i++) {
    			echo '<option value="'.$data_proj[$i]['m_pid'].'" ';
				if( $data_proj[$i]['m_pid']==$_GET['ch_name'])
       ...
       datastr+='&ch_name='+d;
                    ...
                    <table width="70%" border="0" cellspacing="1" cellpadding="1">
                        <tr>
                            ...
                            <td class="grid001">Date of completion</td> 
        ...
        <?php
			  ...
			  ?>
                                <tr>
                                    ...
                                    <td class="gridVal"><?php echo $datalist[$i]['client_name'];?></td>
                                   <td class="gridVal"><?php if($datalist[$i]['date_complet']>0)echo date('m/d/Y',$datalist[$i]['date_complet']);?></td>
                                   <td class="gridVal"><?php echo $datalist[$i]['description'];?></td>
                                    
                                    <td class="gridVal"><a href="add_project.php?pid=<?php echo $datalist[$i]['m_pid'];?>"><img src="<?php echo $mydirectory;?>/images/edit.png" width="24" height="24" alt="edit" /></a></td>
                                   <?php echo '<td align="center" class="gridVal"><a href="project_list.php?del=1&pid=' . $datalist[$i]['m_pid'] . '&' . $paging . '"><img src="' . $mydirectory . '/images/drop.png" width="28" height="28" alt="delete" /></a></td>'; ?>
...
data="";
 ...
 if($.trim($("#ch_name").val())!="")
    ...
    if(data!="")
    location.href='project_list.php'+data;
else
