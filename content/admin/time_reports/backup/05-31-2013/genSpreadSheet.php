...
$sql = 'SELECT  (select city from tbl_store where sto_num=t.store limit 1) as city,t.*,odo.*,e.firstname,e.lastname FROM dtbl_timesheet as t left join "employeeDB" as e on e."employeeID" = t.emp_id left join dtbl_odometer as odo on odo.time_id=t.time_id';  
...
$content .= 'Merchandiser,Client,Store Name & Number,City/Location,Start,in-start,Travel,Miles,Comments,Job Verified,Signoffs,Expenses'."\n";
fwrite($file, $content);
//echo $content;
$content ="";
for($i=0; $i < count($datalist); $i++)
{
   $travel=$datalist[$i]['misc_exp'];
	...
	$content .= '"'.rtrim(str_replace('"','""',$travel)).'",';
        $content .= '"'.rtrim(str_replace('"','""',$datalist[$i]['comment'])).'",';
