...
$sent_to=array();
    ...
    if (isset($report) && $report == 1) {
        $sql = 'SELECT m1.email,prj.prj_name, prj.status from projects as prj left join prj_merchants as merch on merch.pid=prj.pid'.
   ...
   for($i=0;$i<count($sent_to);$i++)
   {
    // echo $sent_to[$i]['email']."<br/>";
      // echo $email_body."<br/>";
       ...
       $headers = create_smtp_headers($subject, "admin@davismerchgroup.com", $sent_to[$i]['email'], $subject, "", "text/html");
