...
if(isset($_GET['st_name'])&&$_GET['st_name']!="")
{
 ...
 $where.=" AND chain.ch_id =".$_GET['st_name']."";   
 $join_type='inner join';
 $cl_flag=1;
}
...
$sql='SELECT distinct (prj.prj_name), 
  main.name, 
  m.st_time, 
  prj.bulb_stat, 
  chain.chain, 
...
FROM 
  public.projects prj, 
  public.prj_merchants_new m, 
  public.project_main main, 
  public.tbl_chain chain, 
  public.tbl_chainmanagement st, 
  public."employeeDB" m1, 
  public.tbl_region reg
WHERE 
  prj.pid = m.pid AND
  m.location = chain.ch_id AND
  m.store_num = st.chain_id AND
  m.merch = m1."employeeID" AND
  m.region = reg.rid AND
  main.m_pid = prj.m_pid  '.$where;
$total =countRec($sql);
$sql .= "  $sort $limit";
