...
if ($sql != '')
{
 ...
 if ($pid == 0)
  ...
  if(isset($pid)&&$pid!=""&&isset($merch_1)&&$merch_1!=0)
    {
 $sql='select m_id from prj_merchants where pid='.$pid.'and  merch='.$merch_1.' and m_id !='.$merch_id_hdn;     
  if (!($result = pg_query($connection, $sql)))
    {
        $ret['error'] = "Failed to add / edit project: " . pg_last_error($connection);
        echo json_encode($ret);
...
$sql.=",address='".pg_escape_string($address)."'";     
 ...
 else{  $sql ='insert into  prj_merchants(pid,cid,merch';   
