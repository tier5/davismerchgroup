...
$sql ='SELECT distinct ( prj.prj_name ), prj.pid, chain.chain,m1.firstname, m1.lastname, m1.region, st.sto_num, m.st_time 
from projects  as prj  
inner join prj_merchants as m on m.m_id = (select mer.m_id from prj_merchants as mer where mer.pid = prj.pid limit 1 )
inner join "employeeDB" as m1 on m1."employeeID"= m.merch left join tbl_store as st on st.sid = m.store_num  '.
 '  left join tbl_chain as chain on chain.ch_id=m.location  '.$where ;
$total =countRec($sql);
$sql .= " $sort $limit";
