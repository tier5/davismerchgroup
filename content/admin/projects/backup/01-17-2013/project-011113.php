...
if ($pid > 0)
{
    ...
    $merch_list=array();
   $sql = "SELECT merch.location,merch.store_num,str2.sto_num as str2_sto_num ,chain.chain,merch.m_id,str.sto_name,str.sto_num,merch.store_num,merch.st_time,merch.cid,merch.merch,merch.notes, merch.address, merch.phone, merch.city, merch.zip, merch.due_date,prj.prj_name as prjname, c.client, c.\"clientID\", m1.firstname as m1first, m1.lastname as m1last ";
...
<table width="95%" >
    <tr>
     ...
     <!--<tr id="u_file" <?php if($pid == 0) echo 'style="display:none;"' ?>>-->
                        <td align="right" <?php if($pid == 0) echo 'style="display:none;"' ?>>File:</td>
                        <td align="left" <?php if($pid == 0) echo 'style="display:none;"' ?>><input type="file" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" name="file" id="file" onchange="javascript:ajaxFileUpload('file');" value="<?php echo stripslashes($datalist['file']); ?>"/></td>
...
<table width="88%" >
  ...
  {
    echo '<tr ><td style="width:150px;padding-left: 10px;"  class="gridVal">' . $merch_list[$i]['m1first'] . ' ' . $merch_list[$i]['m1last'] .
