...
if ($pid > 0)
{
    ...
    $merch_list=array();
   $sql = "SELECT merch.m_id,str.sto_name,merch.store_num,merch.st_time,merch.cid,merch.merch,merch.notes,merch.due_date,prj.prj_name as prjname, c.client, c.\"clientID\", m1.firstname as m1first, m1.lastname as m1last ";
            ...
            <fieldset style="border: 1px solid #333333;" width="80%"><legend><strong>Add Merchandiser</strong></legend>
                <table width="80%">
                  ...
                  echo '>' . $employee[$i]['firstname'] . ' ' . $employee[$i]['lastname'] . '</option>';
                    ...
                    <tr>
                        <td align="right">Start Date:</td><td><input size="15px" type="text" name="due_date" id="due_date" readonly="readonly" value="<?php echo date('m/d/Y',$merch_list[0]['due_date'] );?>" /></td>
                        <td width="20px">&nbsp;</td>
                        <td align="right">Start Time</td><td><input type="text" size="10px" name="st_time" id="st_time"  value="<?php echo $merch_list[0]['st_time'] ?>" /></td>
                        
                    </tr>
                    <tr>
                        <td align="right">Store/Location:</td><td><select name="location" id="location" style="width: 200px;" ><option value="0">--Select--</option>
                                <?php
                    for ($i = 0; $i < count($data); $i++) {
                        echo '<option value="' . $data[$i]['pid'] . '"';
                         if($data[$i]['sto_name']==$merch_list[0]['sto_name'])
                    echo ' selected="selected" ';
                        echo '>' . $data[$i]['sto_name'] . '</option>';
                    }
                    ?> 
                            </select></td>
                        <td width="20px">&nbsp;</td>
                    ...
                    echo ' selected="selected" ';
                      ...
                      <tr>
                        <td align="right">Store#:</td><td><input size="15px" type="text" name="store_num" id="store_num"  value="<?php echo $merch_list[0]['store_num'];?>" /></td>
                        <td width="20px">&nbsp;</td>
                        <td align="right">&nbsp;</td><td>&nbsp;</td>
