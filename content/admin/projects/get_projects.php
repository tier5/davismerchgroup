...
$qr_params=build_http_query($_GET);
...
if(isset($_GET['st_name'])&&$_GET['st_name']!="")
{
 ...
 $where.=" AND mer.location =".$_GET['st_name']."";   
 $join_type='inner join';
 $cl_flag=1;
}
...
if(isset($_GET['sto_num'])&&$_GET['sto_num']!="")
{
 ...
 $where.=" AND mer.store_num =".$_GET['sto_num']."";  
 $join_type='inner join';
 $cl_flag=1;
}
...
if(isset($_GET['region'])&&$_GET['region']!="")
{
 ...
 $where.=" AND mer.region =".$_GET['region']."";   
 $join_type='inner join';
 $cl_flag=1;
}
if($_SESSION['emp_type']!=0) 
...
$where_main.=" prj.pid>0 "; 



$sql ='SELECT distinct ( prj.prj_name ),main.name,prj.bulb_stat, prj.pid,prj.num_merch, chain.chain,m1.firstname, m1.lastname, reg.region, st.sto_num, m.st_time,m.due_date,prj.merch_num_stat 
 from projects'.$ext.'  as prj  
 '.$join_type.' prj_merchants_new'.$ext.' as m on m.m_id = (select mer.m_id from prj_merchants_new'.$ext.' as mer  '
...
if($where != '')
  ...
  $total =countRec("SELECT prj_name from projects".$ext);
$sql .= " $sort $limit";
 ...
 if(isset($_GET['close']) || $_GET['close'] == 1)
{
 ...
 //$row['bulb_stat']='sign';
 if($row['merch_num_stat']==1||$row['merch_num_stat']==''||$row['merch_num_stat']==null)
 {
	...
	$entry = array('id'=>$row['pid'],
		'cell'=>array(
			...
			'reg.region'=>$row['region'],
                        'edit'=>'<a href="project.php?pid='.$row['pid'].($_GET['close']==1?'&close=1':'').'&'.$qr_params.'&page='.$_POST['page'].'"><img src="'.$mydirectory.'/images/'.((($_SESSION['perm_admin']) == "on"||($_SESSION['perm_manager'] == "on"))?'edit.png':'view.png').'" class="hand" alt="Edit" title="Edit: '.$row['prj_name'].'" /></a>',
...
if(!isset($is_client)||$is_client!=1)
{
$entry['cell']['prj.num_merch']=$row['num_merch'];      
}
