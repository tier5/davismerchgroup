...
do
{
    ...
    $datalist=array();
//echo $k.'<br/>';
$sql ='select prj.*,str.sto_name,c.client ,chain.chain,m.*,m1.firstname as m1first, m1.lastname as m1last,str2.sto_num as str2_sto_num  from projects as prj left join "prj_merchants" as m on  prj.pid=m.pid  ';
 ...
 $sql.=" left join tbl_chain as chain on chain.ch_id=m.location ";
  ...
  $sql .= " left join tbl_store as str2 on str2.sid=m.store_num ";
...
//$content .= 'Merchandiser,Start Date,Start Time ,Chain,Store#,Address,Phone,Notes,City,Zip'."\n";
   for($j=0;($datalist[$i][$j][14])!='';$j++)
   {
	...
	$content .= '"'.rtrim(str_replace('"','""',$datalist[$i][$j][32])).'",';
