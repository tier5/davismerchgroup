...
if ($pid > 0)
{
   ...
   // echo $query;
    ...
    $merch_list=array();
   $sql = "SELECT reg.region,merch.location,merch.store_num,  str2.sto_num as str2_sto_num ,chain.chain,merch.m_id,str.sto_name,str.sto_num,merch.store_num,merch.st_time,merch.cid,merch.merch,merch.notes, merch.address, merch.phone, merch.city, merch.zip, merch.due_date,prj.prj_name as prjname, c.client, c.\"clientID\", m1.firstname as m1first, m1.lastname as m1last ";
...
$sql3 = 'Select * from "tbl_chainmanagement" ORDER BY sto_num ASC ';
    ...
    <tr>
        <td>
            <fieldset style="border: 1px solid #333333;" width="80%"><legend><strong>Add Merchandiser</strong></legend>
                <table width="80%">
                    <tr>
                        <td align="right">Merchandiser:<?php echo 'jj'.count($employee);?></td><td><select name="merch_1" id="merch_1" style="width: 200px;" onchange="javascript:get_region();"><option value="0">--Select--</option>
                                <?php
    			...
    			echo '<option value="'.$data_region[$i]['rid'].'" ';
                    ...
                    for ($i = 0; $i < count($data_store); $i++) {
                        echo '<option value="' . $data_store[$i]['chain_id'] . '"';
                         if($data_store[$i]['chain_id']==$merch_list[0]['store_num'])
                    echo ' selected="selected" ';
