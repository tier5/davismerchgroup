...
<tr>
   ...
   $html.='<tr> <td colspan="3"  width="100%">';
if($ralph['smkfsh_img']!='')
{
  if($is_client==1)
{
$html.='<a href="'.$base_url.'content/upload_files/images/'.$ralph['smkfsh_img'].'"><img  width="100px" height="100px" id="proj_sign_img_field" src="'.$image_dir.$ralph['smkfsh_img'].'"/></a>';     
 }   
 else{   
$size = getimagesize($image_dir.$ralph['smkfsh_img']);

                    $html.='<img   ';
   if($size[0]>600) $html.=' width="600px" ';    
   if($size[1]>480) $html.=' height="480px" '; 
                    $html.='   id="proj_sign_img_field" src="'.$image_dir.$ralph['smkfsh_img'].'"/>';
}  }           
   $html.=' </td></tr>';          
  $html.='</table>';          
$html.='<table width="100%" border="0" cellspacing="0" cellpadding="0">
