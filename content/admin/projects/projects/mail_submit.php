...
if (isset($plano) && isset($plano[0]))
{
   ...
   // print_r($plano);
   $html .= '<tr>'.
        '<td><a href="'.$base_url.'content/admin/new_projects/download.php?upid='.$plano[$i]['upid'].'" ></a>'.
 ...
 $html .='<br/><a href="'.$base_url.'content/admin/new_projects/download.php?upid='.$plano[$i]['upid'].'" title="Download '.$plano[$i]['disp_name'].'"><strong>'.$plano[$i]['disp_name'].'</strong></a>';
...
if (isset($cl_sign) && isset($cl_sign[0]))
{
   ...
   $html .= '<tr>'.
        '<td><a href="'.$base_url.'content/admin/new_projects/download.php?upid=" ></a>'.
            '<br/><strong>Client Sign Off:</strong>'.
        '</td>'.
    '</tr>';
    $html .= '<tr>'.
            '<td>';
    for ($i = 0; $i < count($cl_sign); $i++)
    {
               ...
               $html .='<br/><a href="'.$base_url.'content/admin/new_projects/download.php?upid='.$cl_sign[$i]['upid'].'" title="Download '.$cl_sign[$i]['disp_name'].'"><strong>'.$cl_sign[$i]['disp_name'].'</strong></a>';
   ...
   for($i=0;$i<count($sent_to);$i++)
   {
    ...
    // echo $sent_to[$i]['email']."<br/>";
      ...
      // $email_body= htmlentities($email_body);
       ...
       $headers = create_smtp_headers($subject, "davismerchgroup@me.com", $sent_to[$i]['email'], "Davis Merchandising", $sent_to[$i]['name'], "text/html");
       ...
       {$data .= $email_body;}
        ...
        $data .=   "</body></html>";
        //echo $data."<br/>";
       if ((send_smtp($mailServerAddress, "davismerchgroup@me.com", $sent_to[$i]['email'], $data)) == false) {
