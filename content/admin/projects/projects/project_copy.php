...
$columns="created_by";$values=$_SESSION['employeeID'];
     ...
     $i=0;
        foreach($prj_data as $key => $value) {
            if($i%2!=0&&$value!=""&&($key=='m_pid'||$key=='num_merch'||$key=='status'))
            {
      /*   if($columns=='')  
         {
                $columns.=$key;
...
//echo $sql;
 ...
 // print_r($prj_merch);     
    ...
    if($i%2!=0 && $value!="" && ($key=='due_date'||$key=='location'||$key=='cid'))        
            {
         if($columns=='')  
         {
                $columns.=$key;
...
if (!(pg_query($connection, $sql))) {
 ...
 if(isset($_POST['from_merchtab']))
 {
 $res_arr=array('pid'=>$new_pid);
