...
$sql ='SELECT "employeeID" as id, firstname as f, lastname as l,  company_rep FROM "employeeDB" where active=\'yes\' and (emp_type = 0 OR emp_type is null)  ORDER BY lastname ASC ';
//echo $sql;
if(!($result=pg_query($connection,$sql))){
  ...
  <tr>
    <td><fieldset><legend><strong style="font-size:14px;">Search</strong></legend><form accept-charset="utf-8" name="search_frm" id="search_frm" method="post" action="index.php"><table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding-left:20px;">
        <tr>
          ...
          <td>Status: </td>
          <td width="10">&nbsp;</td>
          <td><select style="width:150px;" name="status" id="status">
    ...
    <?php 
    if(isset($_REQUEST['start_dt'])){ 
$date=split(' ',$_REQUEST['start_dt']);
 $dd=split('/',$date[0]);
 $dt=split(':',$date[1]);
    }
 
    ?>
<div class="custom_time_picker"><table><tr><td>Yr</td><td>Mnt</td><td>Dy</td></tr>     
<tr>
<td><select class="cust_time_yr" onchange="changeTime($(this),'date');">    
  <?php  for($i=2000;$i<=2200;$i++){ ?>         
<option <?php if($dd[2]==sprintf("%02s", $i)) echo ' selected';?>><?php echo sprintf("%02s", $i); ?></option>
<?php } ?>
</select></td>
<td><select class="cust_time_mnt" onchange="changeTime($(this),'date');">
  <?php  for($i=1;$i<=12;$i++){ ?>        
<option <?php if($dd[0]==sprintf("%02s", $i)) echo ' selected';?>><?php echo sprintf("%02s", $i); ?></option>
<?php } ?>
</select></td>
<td><select class="cust_time_dy" onchange="changeTime($(this),'date');">
   <?php  for($i=1;$i<=31;$i++){ ?>          
<option <?php if($dd[1]==sprintf("%02s", $i)) echo ' selected';?>><?php echo sprintf("%02s", $i); ?></option>
<?php } ?>
</select></td>
</tr>
 ...
 <?php  for($i=1;$i<=12;$i++){ ?>           
<option <?php if($dt[0]==sprintf("%02s", $i)) echo ' selected';?>><?php echo sprintf("%02s", $i); ?></option>
<?php } ?>
</select></td>
<td><select class="cust_time_min" onchange="changeTime($(this),'date');">
  <?php  for($i=0;$i<60;$i++){ ?>          
<option <?php if($dt[1]==sprintf("%02s", $i)) echo ' selected';?>><?php echo sprintf("%02s", $i); ?></option>
<?php } ?>
</select></td>
<td><select class="cust_time_am" onchange="changeTime($(this),'date');">
<option <?php if($date[2]=='am') echo ' selected';?>>am</option><option <?php if($date[2]=='pm') echo ' selected';?>>pm</option>
</select>
</td></tr>
</table>
 <input  style="display:none;" class="cust_time_text" id="start_dt" name="start_dt" type="text" class="textBox" readonly="readonly" value="<?php if(isset($_REQUEST['start_dt'])){ echo $_REQUEST['start_dt']; }?>" />                           
</div>          
...
<option <?php if($date[2]=='am') echo ' selected';?>>am</option><option <?php if($date[2]=='pm') echo ' selected';?>>pm</option>
 ...
 <input  style="display:none;" class="cust_time_text" id="end_dt" name="end_dt" type="text" class="textBox" readonly="readonly" value="<?php if(isset($_REQUEST['end_dt'])){ echo $_REQUEST['end_dt']; }?>" />                           
</div>
 ...
 <input class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all"  type="submit" value="Approved Items" /> 
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
		...
		foreach ($time as $t)
		{
			...
			<td align="left" valign="top" class="gridVal"><?php if($t['store']==0) { echo $t['others']; } else echo $t['ch']; ?> </td>
		 ...
		 echo 	'<tr>
			<td width="100%" height="30" class="grid001" colspan="15">'.$p->show().'</td>			
		  </tr>';
		}
		else
		{
			echo '<tr><td colspan="16" height="30" class="grid001">No Time Record found</td><tr>';
		}
	   ?>       
      </table></form></td>
  </tr>
...
{
  showLoading();
  data='time_id='+$('#employee').val()+'&start_dt='+$('#start_dt').val()+'&end_dt='+$('#end_dt').val()+'&status='+$('#status').val();
  $.ajax({type: 'POST',
