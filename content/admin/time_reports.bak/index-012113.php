...
$sql="Select (select city from tbl_store where sid=t.store limit 1) as city,t.time_id,t.store,ch.chain as ch, t.start_time, t.end_time, t.hours_worked,t.lunch_hrs, t.reg_hours, t.ot_hours, t.status, e.firstname as f, e.lastname as l,reg.region as r,odo.daily_total, odo.reimburse_mile, odo.gas_rec, odo.misc_exp, odo.misc_exp_type from dtbl_timesheet t ".
"left join tbl_chain as ch on ch.ch_id=t.store  left join \"employeeDB\" as e on e.\"employeeID\" = emp_id left join tbl_region as reg on reg.rid=e.region left join dtbl_odometer as odo on odo.time_id=t.time_id   where ".$search_sql." ORDER BY time_id DESC";
//echo $sql;
if(!($result=pg_query($connection,$sql))){
	print("Failed query1: " . pg_last_error($connection));
	exit;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <input  class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" onclick="javascript:payRoll(12);" type="button" value="Payroll Report" />           
          </td>
        </tr>
        <tr>
          <td>&nbsp;</td>
 ...
 <input class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all"  type="submit" value="Approved Items" /> 
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
		...
		foreach ($time as $t)
		{
			...
			echo '<td align="left" valign="top" class="gridVal">'.$t['ch'].'</td>';
                        echo '<td align="left" valign="top" class="gridVal">'.$t['city'].'</td>';
		...
		{
			echo '<tr><td colspan="15" height="30" class="grid001">No Time Record found</td><tr>';
		}
	   ?>       
      </table></form></td>
  </tr>
