...
$sql = 'SELECT  store.sto_num as store_num_val,ch.chain,t.*,t.other as others,t.store as store, odo.*,e.firstname,e.lastname,odo.daily_total as daily_total,odo.other_exp as other_exp,e.city as city FROM dtbl_timesheet as t left join "employeeDB" as e on e."employeeID" = t.emp_id left join dtbl_odometer as odo on odo.time_id=t.time_id  ';  
$sql.=' left join "tbl_store" as store on store.sid=t.store_num'
//$sql.=' left join "tbl_store" as store on store.sid=t.time_id'
