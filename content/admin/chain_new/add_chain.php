...
if (isset($_GET['chain_id'])) {
    ...
    $sql = "select * from tbl_chainmanagement where  chain_id = $chain_id";
...
if ($isEdit) {
    $query = ("SELECT * from tbl_chainmanagement " .
...
<table width="90%">
    <tr>
        <td align="left">
            <input type="button" value="Back" onclick="location.href ='chain_new_list.php'" />
        </td>  
        <td>&nbsp;</td>
    </tr>
</table>
...
<form action="chain_submit.php" id="store_frm" method="post">
  <table align="center">
           ...
           <td align="right">Chain:</td>
			...
			for ($i = 0; $i < count($data_chain); $i++) {
    			echo '<option value="'.$data_chain[$i]['ch_id'].'" ';
    				if (isset($datalist['sto_name']) && $datalist['sto_name'] == $data_chain[$i]['ch_id'])
        ...
        <tr>
            ...
            <td><input type="text" name="store" value="<?php echo stripslashes($datalist['sto_num']); ?>" size="20"> <input type="hidden" name="chain_id" value= "<?php 
			if(isset($_GET['chain_id']) && $_GET['chain_id']!='') echo $_GET['chain_id']; ?>"  /></td>
        </tr>
        <tr>
