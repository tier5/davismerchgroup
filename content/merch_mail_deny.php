...
//echo $html;
        ...
        $subject = 'Davis Merchandising : Job denied by '.$data_prj[0]['m1first']." ". $data_prj[0]['m1last'];
...
//						  }
        for ($i = 0; $i < count($data_prj); $i++) {
            ...
            $email_body[$i] = "<p>The merchandiser ".$data_prj[$i]['m1first']." ". $data_prj[$i]['m1last']." denied the job in ".date('m/d/Y', $data_prj[$i]['due_date'])."</p>";
   ...
   if ((isset($data_prj[$i]['m1last']) && $data_prj[$i]['m1last'] != "")||(isset($data_prj[$i]['m1first']) && $data_prj[$i]['m1first'] != ""))
            ...
            if (isset($deny_reason) && $deny_reason != "")
                $email_body[$i] .= "<p><strong>Reason:</strong> " . $deny_reason . " </p>";
        $email_body[$i].=$html;
   ...
   for($i=0;$i<count($sent_to);$i++)
   {
    ...
    // echo $sent_to[$i]['email']."<br/>";
      ...
      // $email_body= htmlentities($email_body);
       ...
       $headers = create_smtp_headers($subject, 'admin@davismerchgroup.com','davismerchgroup@me.com', "Davis Merchandising", $sent_to[$i]['name'], "text/html");
       ...
       {$data .= $email_body;}
        ...
        $data .=   "</body></html>";
        //echo $data."<br/>";
       if ((send_smtp($mailServerAddress, "admin@davismerchgroup.com",'davismerchgroup@me.com', $data)) == false) {
