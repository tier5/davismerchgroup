...
if (isset($_SESSION['emp_type']) && $_SESSION['emp_type'] != '' && $_SESSION['emp_type'] == 1){
	...
	echo "<tr valign=\"top\">";
	echo "<td align=\"center\"><a href=\"timesheet\"><img src=\"images/tmsheet.gif\" border=\"0\" alt=\"Time Clock\"></a></td>";
	echo "<td align=\"center\"><a href=\"admin/time_reports\"><img src=\"images/timecardreports.png\" border=\"0\" alt=\"Reports\"></a></td>";
	echo "<td align=\"center\"><a href=\"admin/EmployeeDetails/listApplication.php\"><img src=\"images/btn2.jpg\" border=\"0\" alt=\"Employee Applications\"></a></td>";
	echo "<td align=\"center\"><a href=\"\"><img src=\"images/btn3.jpg\" border=\"0\" alt=\"Prospects\"></a></td>";
	echo "<td align=\"center\"><a href=\"admin/projects\"><img src=\"images/btn4.jpg\" border=\"0\" alt=\"Projects\"></a></td>";
	echo "</tr>";
	echo "<tr>";
	echo "<td align=\"center\"><a href=\"timeclock/index.php\"><img src=\"images/emp_timeclock.png\" border=\"0\" alt=\"Employee Timeclock\"></a></td>";
	echo "<td align=\"center\"><a href=\"admin/email_weekly/index.php\"><img src=\"images/emailschdule.png\" border=\"0\" alt=\"Weekly email\"></a></td>";
	echo "<td align=\"center\"><a href=\"admin/signoff_report/index.php\"><img src=\"images/signoffreports.png\" border=\"0\" alt=\"Projects\"></a></td>";
	echo "<td align=\"center\"><a href=\"distance/index.php\"><img src=\"images/timereport.png\" border=\"0\" alt=\"Distance\"></a></td>";
	...
	$sql = "SELECT distinct (name ),m_pid as pid ".
		"FROM project_main ".
		"ORDER by name ";
	...
	$sql = "SELECT prj.prj_name ,m1.firstname, m1.lastname, main.name, m.confirm, m.merch, m.m_id, m.city, prj.pid, ch.chain, m.view_stat, st.sto_num, m.st_time, m.due_date ".
		...
		"LEFT JOIN tbl_chain as ch ON ch.ch_id=m.location ".
		"LEFT JOIN tbl_chainmanagement as st on st.chain_id = m.store_num ".
		"LEFT JOIN project_main as main on main.m_pid=prj.m_pid ".
		"LEFT JOIN \"employeeDB\" as m1 on m1.\"employeeID\"= m.merch ".
		"WHERE m.pid = prj.pid AND m.\"due_date\" >= $this_w_st AND m.\"due_date\" <= ". $this_w_end . $srch_sql ." AND prj.status = 1 ".
		"ORDER BY m.m_id asc";
 ...
 */
	...
	echo "<script type=\"text/javascript\">";
		echo "var from_proj=1; ";
		echo "var from_report=0; ";
	echo "</script>";
		
	echo "<div style=\"border-color:black;border-style:solid;border-width:3px;\">";
	echo "<table style=\"margin:0px;padding:0px;\" width=\"100%\">";
	echo "<tr>";
	echo "<td colspan=\"3\" align=\"center\"><h3>Welcome To the Internal Intranet for Davis Merchandising Group</h3></td>";
	echo "</tr>";
...
<?php 
					if ($week['merch'] == $_SESSION['employeeID'] && ($_SESSION['perm_admin'] == "on" || $_SESSION['perm_manager'] == "on"))
					{ 
						if($m_prev_f==0)
						{
?> 
...
?>
						...
						<table width="100%">   
							<tr>
								<td>Accept</td><td><input <?php if ($week['confirm'] == 't') echo' checked '; ?> onchange="merch_stat_submit('<?php echo $week['m_id']; ?>',$(this).val());" value="yes" type="radio" name="status_<?php echo $cnt; ?>"/></td>
							</tr>
							<tr>	
								<td>Deny</td><td><input <?php if ($week['confirm'] == 'f') echo' checked '; ?> onchange="merch_stat_submit('<?php echo $week['m_id']; ?>',$(this).val());" value="no" type="radio" name="status_<?php echo $cnt; ?>"/></td>
							</tr>
					    	</table>
...
?>
					...
					<td align="center"><img width="20" height="20" src="
...
?>
						...
						/images/<?php echo $bulb_clr; ?>" style="cursor:pointer;cursor:hand;" />
					</td>
				</tr>    
...
?>
				...
				<tr>
					<td colspan="12"><?php if ($p_num > 0) echo $p->show(); ?></td>
				</tr>
                    </table>
                </td>   
                <td width="2%">&nbsp;</td> 
...
?>
				...
				<tr style="background-color:#24ABED;">
					<td colspan="12" align="center" style="color:white;"><font size="3"><b>Upcoming Schedule</b></font></td>
				</tr>
				<tr>
					<td colspan="12"><?php if ($p2_num > 0) echo $p2->show(); ?></td>
				</tr>    
                        	<tr style="background-color:#CCC;">
					<td style="color:black;"><strong>Start Date</strong></td>
					<td style="color:black;"><strong>Employee Name</strong></td>
					<td style="color:black;"><strong>Project Name</strong></td>  
					<td style="color:black;"><strong>Job ID</strong></td>  
					<td style="color:black;"><strong>Store</strong></td>  
                            		<td style="color:black;"><strong>Store&nbsp;#</strong></td> 
					<td style="color:black;"><strong>Location</strong></td>
					<td style="color:black;"><strong>Start Time</strong></td>  
					<td style="color:black;"><strong>View</strong></td>  
                            		<td style="color:black;"><strong>Time Entry</strong></td>    
                            		<td style="color:black;"><strong>Confirm</strong></td>  
                            		<td style="color:black;"><strong>Status</strong></td>  
                        	</tr>
...
?>
				...
				<tr 
<?php 
					if($m_prev_f==1) echo ' style="display:none;" ';
?> 
					height="30px" 
<?php 
					if ($c_flag == 1)
...
?>
				>
                                	<td>
<?php 
						echo date('l, d- M', $week['due_date']); 
?>
					</td> 
                                	<td>
<?php 
						echo $week['firstname'] . ' ' . $week['lastname'] 
?>
					</td> 
                                	<td>
<?php 
...
?>                
                                    			<table width="100%">   
                                        			<tr>
									<td>Accept</td>
									<td><input <?php if ($week['confirm'] == 't') echo' checked '; ?> onchange="merch_stat_submit('<?php echo $week['m_id']; ?>',$(this).val());" value="yes" type="radio" name="status_<?php echo $cnt; ?>"/></td>
								</tr>
                                        			<tr>
									<td>Deny</td><td><input <?php if ($week['confirm'] == 'f') echo' checked '; ?> onchange="merch_stat_submit('<?php echo $week['m_id']; ?>',$(this).val());" value="no" type="radio" name="status_<?php echo $cnt; ?>"/></td>
								</tr>
...
<div id='sub_content_confirm' style="display:none;">
	<form id='sub_content_confirm_form'>    
        <table width="100%">  
        	<tr>
			<td>State your reason for the denying the job.</td>
		</tr>    
		<tr>
			<td><textarea id="deny_reason" name="deny_reason" cols="75" rows="10"></textarea></td>
		</tr> 
		<tr>
			<td><input class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="button" value="Submit" onclick="javascript:submit_merch_deny();"/></td>
		</tr> 
    	</table>
	</form>  
</div>    
<link rel="stylesheet" type="text/css" href="<?php echo $mydirectory; ?>/css/jquery.jgrowl.css" />    
<style type="text/css">
...
?>
			...
			header: "
<?php 
				echo str_replace(PHP_EOL,'',$anc['title']."&nbsp;&nbsp;<a style='color:#24ABED;' target='_blank' href='./announcement/index.php?edit=".$anc['anc_id']."'>Read more..</a>"); 
?>
				", sticky: true,position:'center'});
<?php  // no idea what these are closing but dont want to take them out... Man I Love shitty programmers
		}
