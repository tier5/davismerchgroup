...
var calendar = $('#calendar').fullCalendar({
	theme: true,
	editable: false,
	events: "month_view.php?user="+$('#user').val(),
...
showLoading();
$.ajax({type: 'POST',url: 'addtime.php?user='+$('#user').val(),data: dataString,dataType: 'html',success: function(html){
...
function calculate_hrs(dateText){
   ...
   if(hrs_diff > 0){	 
	 ...
	 if(hrs_diff >= 6){$('#lunch').removeClass('tr_hide');if($('#lunch_st').val() == ''){$('#lunch_st').val('12:00 pm');$('#lunch_ed').val('12:30 pm');}}else if(!$('#lunch').hasClass('tr_hide')){$('#lunch').addClass('tr_hide');$('#lunch_st').val('');$('#lunch_ed').val('');} 
...
function calculate_misc(){
	...
	$('#odo_mis_exp').val(Math.round((sum*100)/100));
}
function isNumber(n) {
